...
InitializeDatabaseSocket: function(socket, username, permissions) {
	...
	socket.on('AddDirectory', function(path, name, creator) {
		...
		if(dir) {
			for(var i = 0; i < databaseSockets.length; i++) {
				databaseSockets[i][0].emit('UpdateDatabase', 'add', {path: path, entry: dir});
			}
		} else {
	...
	socket.on('DeleteEntry', function(path) {
		...
		if(!(user[2] === 'Admin' || user[1] === dir.creator)) {
			socket.emit('UpdateDatabase', 'You do not have permission to delete that!');
			return;
		}
		var success = that.DeleteEntry(path);
		if(success) {
			for(var i = 0; i < databaseSockets.length; i++) {
				databaseSockets[i][0].emit('UpdateDatabase', 'del', {path: path});
			}
		} else {
	...
	socket.on('EditEntry', function(path, name, content) {
		var dir = that.GetDirectoryFromPath(path);
		if(!(user[2] === 'Admin' || user[1] === dir.creator)) {
			socket.emit('UpdateDatabase', 'You do not have permission to edit that!');
			return;
		}
		var success = false;
		if(content) {
		...
		if(success) {
			for(var i = 0; i < databaseSockets.length; i++) {
				databaseSockets[i][0].emit('UpdateDatabase', 'edit', {path: path, name: name});
			}
		}
	});
